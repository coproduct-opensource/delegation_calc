...
Yet a vulnerability surfaced in February 2026 on the IETF OAuth mailing
...
\item \textbf{In the cryptographic correspondence (T2; target
  strength, open)}: an EUF-CMA adversary spanning a spliced chain
  should reduce to forgery of one of the two underlying signatures.
  Section~\ref{sec:computational} states this reduction as an open
  obligation; what is proven today is the axiom-free symbolic
  characterization (crypto typing $\iff$ logical typing $\wedge$
  signature validity).
...
\item A modal-linear-temporal authorization logic with a no-chain-
  ...
  \texttt{sorry} on the proofs that are closed: T1 propositional
  decidability (soundness and completeness of the checker, with a
  \texttt{Decidable} instance), the T2 symbolic characterization,
  graded comonad laws, indexed monad laws, and the structural
  substitution lemmas. Statuses are recorded in a machine-validated
  ledger (\texttt{lean/theorem-status.json}); CI rejects any document
  claiming more.
...
\item A Rust implementation of the logic kernel and wire format with
  80 unit tests, including a CBOR codec with round-trip-tested
  encoding for all 22 term constructors and Tamarin/ProVerif fact
  exporters.  (The end-to-end verifier and the cryptographic
  operations are stubs at this revision; the ledger reports
  \texttt{implemented: false} until they are real.)
...
\item An honest assessment (Section~\ref{sec:conclusion}) of what is
  ...
  engagement.  We make no claim to having closed T2 beyond its
  axiom-free symbolic characterization, and no claim to any form of
  T3: the current T3 lemmas are one-run reflexivity infrastructure
  for the logical relation, not non-interference.  Documented paths
  to closure run via VCVio~\cite{tuma2026vcvio} for T2's
  computational half and a two-run logical relation with a
  fundamental lemma for T3.
\end{enumerate}
