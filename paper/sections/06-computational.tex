...
\begin{enumerate}
\item \textbf{EasyCrypt.}  \texttt{models/easycrypt/Game.eca} is a
  skeleton only: type declarations, signing-oracle and game module
  shapes, and the game-hop sequence as comments.  It contains no
  proof content --- the placeholder axiom's body is literally
  \texttt{true}, so it encodes no cryptographic assumption, and no
  reduction lemma has a proof body (CI parse-checks via
  \texttt{easycrypt -batch}, nothing more).  Closure requires
  cryptographer pairing (Blanchet was the named collaborator in our
  plan).
